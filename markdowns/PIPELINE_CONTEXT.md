...
| `pipeline/config.py` | Pipeline-wide constants: `ANTHROPIC_MODEL` (Sonnet as of Round 5 — default for A1, A2, A3, A5, A6), `ANTHROPIC_SYNTHESIS_MODEL` (Sonnet — used exclusively by A4 `fields_generator`), `ANTHROPIC_MAX_TOKENS`, `EXPERIENCE_GAP_BLOCK_THRESHOLD_YEARS`, `WORD_COUNT_TOLERANCE_PCT`. All non-field-editor agents import their model constant from this module. |
| `pipeline/validators.py` | Hard-block output validators between agent stages. `validate_fields_generator_output` raises `PipelineValidationError` if every `generated_fields[].content` is empty after A4, halting the pipeline before A5 runs. |
| `cv_extractor.py` — Round 4 schema additions | As of Round 4, `cv_data.json` carries three new optional fields: `language_scale_direction` (Literal `"1_best"` / `"1_worst"` / null — set by A1 when a numeric scale direction header is detected), `references` (list of `Reference` objects with name/title/organisation/email/phone — extracted when a References section is present), `certification_declaration` (str — verbatim certification block when present). All default to null / `[]` / `""` for backward compatibility. |
| `cv_extractor.py` — Round 6 prompt additions (Fix V, Fix W) | `SYSTEM_PROMPT_A1` extended with two new sections: `### Merged-cell and two-column project tables` (Fix V — instructs A1 to treat left-column text as `project_name`; emit `extraction_warnings` when indeterminate; never fabricate) and `### Date ordering validation` (Fix W — instructs A1 to swap inverted `date_from`/`date_to` across all four date-field types and emit `extraction_warnings` for each swap). |
| `cv_extractor.py` — Round 6 employment fallback (Fix CC) | `SYSTEM_PROMPT_A1` extended with `### Employment-only fallback (all formats)`: when `relevant_projects` is empty and `employment_record` has entries, each employment entry is mapped to a `RelevantProject` — `description → main_project_features`, `employer → project_name + company`, `activities_performed / client / donor = ""`. Python safety net `_apply_employment_fallback` mirrors the same routing. Short-description entries emit a per-entry `extraction_warnings` referencing `main_project_features`. |
...
| `pipeline/agents/cv_tor_mapper.py` — Round 7 (Fix EE) | After `_enforce_threshold_and_cap`, `_sort_by_date_desc` sorts both `relevant_projects` and `countries_of_experience` descending by `date_from` (tie-break: `date_to`) using `_parse_date` from `precompute_utils.py`. Sort applied before `CVData.model_validate` so `mapped_cv.json` is always in newest-first order when A4 generates `detailed_tasks`. Guarantees WB positional pairing (`detailed_tasks[i]` ↔ `relevant_projects[i]`) without any change to `wb.py`. |
| `pipeline/agents/fields_generator.py` — pre-processing (Round 7) | Before A4: `_precompute_project_dates` fills `duration`/`year` (pre-compute moved upstream to `cv_tor_mapper` in Round 5 — this is now a no-op stub). Fix JJ (Round 7): `_truncate_project_text_for_a4`, `_restore_truncated_project_text`, `A4_INPUT_PROJECT_WORD_CAP`, and `cv_data_full` preservation removed entirely. A4 receives full untruncated project text directly from `mapped_cv.json`. `claude-sonnet-4-6`'s 200k context window makes the 150-word cap unnecessary. |
| `pipeline/agents/compressor.py` — Round 7 updates (Fix KK + Fix LL) | **Fix KK**: `_truncate_project_text_for_a6`, `A6_INPUT_PROJECT_WORD_CAP`, and all `input_field_truncated` manifest warning emissions removed entirely. A6 now receives full untruncated project text. **Fix LL**: donor-aware exclusion added — for GIZ runs, `activities_performed` is cleared on all projects in `cv_data_for_a6` before the A6 call (field is not rendered in GIZ output) and restored from `cv_data_in` after A6 returns. Word counts for GIZ exclude `activities_performed`. WB runs are unchanged. |
...
| **2** | `run_phase2` | **`cv_tor_mapper`** (`_run_if_needed` skips if manifest already `done`). Before the LLM call: `_precompute_project_dates_for_mapper` pre-fills `duration`/`year` on all projects (moved upstream from `fields_generator` in Round 5); `_precompute_relevance_scores` computes Python keyword-overlap and geography scores injected as `<pre_computed>` into the A3 user message. After the LLM returns: `_enforce_threshold_and_cap` enforces thresholds (0.30/0.40/0.50 by project count), floor (`min(MIN_PROJECTS_TO_KEEP, total)`), and cap (`MAX_PROJECTS_TO_KEEP`). Only `data.relevant_projects` is affected; the full `project_scores` alignment is preserved. | `checkpoint_2_pending` |
| **3** | `run_phase3` | **`fields_generator`** → **`validate_fields_generator_output`** (hard-block: halts with `set_failed` + `fields_generator` step overridden to `failed` if all `generated_fields[].content` are empty) → **`content_reviewer`** (non-blocking; records review in `generated_fields.json`) → **`compressor`** | `checkpoint_3_pending` or `failed` |
...
**Post-completion field-edit** (`status` must be **`completed`**): **`POST /sessions/{id}/field-edit`** calls [`increment_round`](api/services/database.py) (DB `sessions.round` **is** incremented), then `set_processing`, then runs `run_field_editor_task` **synchronously** inside the HTTP handler. `run_field_editor_task` calls `field_editor.run`, resets the `checkpoint_3` manifest step to `pending` and `renderer` to `waiting`, then calls `set_checkpoint_pending(session_id, 3)`. The HTTP response returns `status: "checkpoint_3_pending"` with `applied`/`skipped`/`round` lists and a `kq_source` field (`"ai_generated"` | `"extracted"` | `"absent"`) indicating which data source provided the key qualification bullets that edits targeted (computed from the post-edit state of `generated_fields.json`). Approving `checkpoint_3` re-runs Phase 4; `build_object_path` uses the incremented `round` from the DB, producing `round_02_giz.docx`, `round_03_giz.docx`, etc.
