...
| `cv_extractor.py` — Round 7 employment fallback (Fix CC) | `SYSTEM_PROMPT_A1` extended with `### Employment-only fallback (all formats)`: when `relevant_projects` is empty and `employment_record` has entries, each employment entry is mapped to a `RelevantProject` — `description → main_project_features`, `employer → project_name + company`, `activities_performed / client / donor = ""`. Python safety net `_apply_employment_fallback` mirrors the same routing. Short-description entries emit a per-entry `extraction_warnings` referencing `main_project_features`. |
...
| `pipeline/agents/fields_generator.py` — pre-processing + post-processing | Before A4: (1) `_precompute_project_dates` fills `duration`/`year`; (2) `cv_data_full` reference preserved (pre-truncation); (3) `_truncate_project_text_for_a4` trims `activities_performed` and `main_project_features` to `A4_INPUT_PROJECT_WORD_CAP = 150` words with a `"…"` suffix for the A4 user message only. After A4 returns: `_restore_truncated_project_text(cv_data_out, cv_data_full)` restores the original untruncated text before writing `generated_fields.json`. `mapped_cv.json` on disk is never affected. |
| `pipeline/agents/compressor.py` — Fix Z pre-processing (Round 6) | Before A6 LLM call: `_truncate_project_text_for_a6` caps `activities_performed` and `main_project_features` to `A6_INPUT_PROJECT_WORD_CAP = 150` words per project. Unlike Fix 8 Part 3 (A4), there is **no restoration step** — A6 is explicitly compressing these fields and the capped text is the intended input. Truncation events are emitted as `input_field_truncated` manifest warnings via `append_warning`. Word counts passed to A6 are recomputed on the truncated copy. |
